    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult List()
        {
            using(var context = new FoodieDBContext())
            {
                ...
                {
                    order.Product = context.Products.FirstOrDefault(p => p.ProductId == order.ProductId);
                    order.Payment = context.Payments.FirstOrDefault(pay => pay.PaymentId == order.PaymentId);
                }
                ViewBag.orders = orders;

                for (int i = 0; i < orders.Count(); i++)
                {
                    for (int j = 1; j < orders.Count(); i++){
                        orders.IndexOf(i).;
                    }
                }
            }
